...
need_adj = (
    ...
    or (args.method == 'MLP' and args.gem and not args.batch_size)
    or args.method == 'LPA'
    or args.method == 'ECN'
    or args.gem
...
for run in range(args.runs):
    ...
    if args.kd:
        ...
        net = gpu(nn.ModuleDict(dict(
            layer1=gnn.MLP([
                n_features,
                *([args.hidden] * (args.n_layers - 2)),
                args.hidden * 2
            ], dropout=args.dropout, norm='layer_norm'),
            ...
            skip_layer=nn.Sequential(
                ...
                nn.Linear(args.hidden, n_labels),
                # nn.LogSoftmax(dim=-1),
            ) if args.skip_connection else nn.Identity(),
        )))
        layer1 = lambda x: net.layer1(x).view(*x.shape[:-1], 2, -1)
        opt = Optim([*net.parameters()])
        e = A.indices()
        induc_e = induc_A.indices()
        ...
        for epoch in range(1, 1 + args.max_epochs):
            ...
            for eb, q in edge_sampling(induc_e, induc_p, bs, True):
                with opt:
                    ...
                    h = layer1(xs)
                    h = torch.cat((h[:, :, :1], h[[1, 0], :, 1:]), dim=2)
                    z = lsg(net.layer2(h))
                    if args.skip_connection:
                        z = torch.cat((
                            z[:, :, :1], z[:, :, 1:] + lsg(net.skip_layer(h[:, :, :1]))
                        ), dim=2)
                    yp = sg(z.detach() / args.alpha)
                    loss = 0
                    if ms.any().item():
                        tys = induc_Y[eb[ms]]
                        yp[ms, :, tys] = 1
                    ...
                    loss = loss - (
                        args.gem * (
                            z[:, :, 1] * yp[:, :, 0]).sum(dim=-1).mean()
                        + args.beta * (
                            z[:, :, 0] * yp[:, :, 1]).sum(dim=-1).mean())
                    if args.embedding:
                        loss = loss + args.embedding * F.cosine_similarity(
                            h[0, :, 1], h[1, torch.randperm(h.shape[1]), 1]).mean()
                    loss.backward()
                ev.record_training(opt.elapsed)
            t = time.time()
            with torch.no_grad():
                net.eval()
                z = net.layer2(layer1(x)[:, 0])
                has_cold = False
            ev.record_evaluation(time.time() - t)
            if ev.evaluate_result(z):
                break
        ...
        for epoch in range(1, 1 + args.max_epochs):
            ...
            if args.run == 0:
                A @ x  # NOTE: This is crucial for the speed evaluation of EEM
            t = time.time()
            with torch.no_grad():
                net.eval()
                z = net(x, E)
